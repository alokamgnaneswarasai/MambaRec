...
class HierarchicalTransformer(nn.Module):
    def __init__(self, user_num, item_num, args):
        ...
        # Positional encoding
        self.pos_emb = nn.Embedding(self.segment_len, self.hidden_units)

        # Transformer layers
        self.first_level_transformer = nn.Transformer(
            d_model=self.hidden_units,
            nhead=self.num_heads,
            num_encoder_layers=self.num_blocks,
            dropout=self.dropout_rate,
        )
        self.second_level_transformer = nn.Transformer(
            d_model=self.hidden_units,
            nhead=self.num_heads,
            num_encoder_layers=self.num_blocks,
            dropout=self.dropout_rate,
        ...
        self.final_layernorm = nn.LayerNorm(self.hidden_units, eps=1e-8)

    def log2feats(self, log_seqs):
        """
        Processes input sequences into feature representations using a hierarchical Transformer.
        """
        batch_size, seq_len = log_seqs.shape

        # Embedding lookup and dropout
        seqs = self.item_emb(log_seqs)  # (B, T, H)
        seqs *= self.hidden_units ** 0.5  # Scale embeddings
        seqs = self.emb_dropout(seqs)

        # Split into overlapping segments
        stride = self.segment_len - self.overlap_size
        segments = []
        for i in range(0, seq_len - self.segment_len + 1, stride):
            segments.append(seqs[:, i:i + self.segment_len, :])
        segments = torch.stack(segments, dim=1)  # (B, S, L, H)

        num_segments = segments.size(1)
        segments = segments.reshape(-1, self.segment_len, self.hidden_units)  # Flatten segments for processing

        # Add positional embeddings to each segment
        pos_indices = torch.arange(self.segment_len).unsqueeze(0).to(self.device)
        segments += self.pos_emb(pos_indices)  # (B * S, L, H)

        # Process each segment using the first-level Transformer
        segments = segments.transpose(0, 1)  # (L, B * S, H) for Transformer
        segments = self.first_level_transformer(segments)  # (L, B * S, H)
        segments = segments.transpose(0, 1).reshape(batch_size, num_segments, self.segment_len, self.hidden_units)
        segment_features = segments.mean(dim=2)  # Aggregate features per segment (B, S, H)

        # Process aggregated features using the second-level Transformer
        segment_features = segment_features.transpose(0, 1)  # (S, B, H)
        high_level_features = self.second_level_transformer(segment_features)  # (S, B, H)
        high_level_features = high_level_features.transpose(0, 1)  # (B, S, H)

        # Normalize and return features
        log_feats = self.final_layernorm(high_level_features)  # (B, S, H)
        return log_feats
    ...
    def forward(self, user_ids, log_seqs, pos_seqs, neg_seqs):  # For training
        ...
        pos_logits = (log_feats[:, -1, :] * pos_embs).sum(dim=-1)
        neg_logits = (log_feats[:, -1, :] * neg_embs).sum(dim=-1)

        return pos_logits, neg_logits

    def predict(self, user_ids, log_seqs, item_indices):  # For inference
        log_feats = self.log2feats(log_seqs)
        final_feat = log_feats[:, -1, :]  # Use the last feature vector
