...
from slender_mamba.utils import contiguous
from slender_mamba.ops.layernorm import RMSNorm

def activation_quant(x):
    """
    # Code from https://github.com/ridgerchu/matmulfreellm/blob/master/mmfreelm/ops/fusedbitnet.py
...
def _layer_norm_fwd_quant_kernel(
    ...
    y = tl.where(y * scale - tl.math.floor(y * scale) < 0.5, tl.math.floor(y * scale), tl.math.ceil(y * scale))
...
def _layer_norm_fwd_quant(
   ...
   # Ensure all tensors are on the same device as `x`
    ...
    # Allocate output tensors on the correct device
    y = torch.empty_like(x, dtype=x.dtype if out_dtype is None else out_dtype, device=device)
    if residual is not None or (residual_dtype is not None and residual_dtype != x.dtype):
        residual_out = torch.empty(M, N, device=device, dtype=residual_dtype)
    else:
        residual_out = None
    mean = torch.empty((M,), dtype=torch.float32, device=device) if not is_rms_norm else None
...
def _layer_norm_bwd(
    ...
    device = x.device  # Ensure all tensors are on the same device as `x`
    dy = dy.to(device)
    weight = weight.to(device) if weight is not None else None
    bias = bias.to(device) if bias is not None else None

    if dresidual is not None:
        dresidual = dresidual.to(device)

    # Allocate output tensors on the correct device
    dx = torch.empty_like(x, dtype=x_dtype, device=device)
    dresidual_in = (
        torch.empty_like(x, device=device) if has_residual and dx.dtype != x.dtype else None
    )
    y = (
        torch.empty_like(dy, device=device) if recompute_output else None
    )
    # added above
...
class LayerNormLinearQuantFn(torch.autograd.Function):
    ...
    def forward(
        ...
        linear_weight = weight_quant(linear_weight).to(dtype).to(y.device)
        #linear_weight=linear_weight.to(dtype)ste
...
class BitEmbedding158b(nn.Module):
    def __init__(self, num_embeddings, embedding_dim, padding_idx=None, max_norm=None, norm_type=2.0,
                 ...
                 bias=False, rms_norm_eps=1e-6):
        super(BitEmbedding158b, self).__init__()
        self.embedding = nn.Embedding(num_embeddings, embedding_dim, padding_idx, max_norm, norm_type,
                                      scale_grad_by_freq, sparse, _weight, device, dtype)
        self.norm = nn.LayerNorm(embedding_dim, eps=rms_norm_eps)
    ...
    def forward(self, input):
        return LayerNormEmbeddingQuantFn.apply(
            input,
            self.norm.weight,
            self.norm.bias,
    ...
    def extra_repr(self) -> str:
        return f'num_embeddings={self.embedding.num_embeddings}, embedding_dim={self.embedding.embedding_dim}, bias={self.bias is not None}'

class LayerNormEmbeddingQuantFn(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, norm_weight, norm_bias, embedding_weight, embedding_bias, eps):
        # Quantize embedding weights
        # print(f"Type of embedding_weight: {type(embedding_weight)}")
        if not isinstance(embedding_weight, torch.Tensor):
            raise TypeError("embedding_weight must be a torch.Tensor")
        quantized_embedding_weight = weight_quant(embedding_weight).to(x.device)
        #quantized_embedding_weight=embedding_weight
        # Perform embedding lookup with quantized weights
        embeddings = F.embedding(x, quantized_embedding_weight)
        
        # Reshape embeddings to 2D: (batch_size * seq_len, embedding_dim)
        original_shape = embeddings.shape
        embeddings_2d = embeddings.view(-1, embeddings.size(-1))
        
        # Apply layer normalization and quantization using Triton kernel
        y, mean, rstd, _ = _layer_norm_fwd_quant(
            embeddings_2d, norm_weight, norm_bias, eps,
            residual=None, out_dtype=None, residual_dtype=None, is_rms_norm=False
        )
        
        # Reshape y back to original shape
        y = y.view(original_shape)
        
        ctx.save_for_backward(x, embedding_weight, norm_weight, norm_bias, embedding_bias, mean, rstd)
        ctx.eps = eps
    ...
    def backward(ctx, grad_output):
        x, embedding_weight, norm_weight, norm_bias, embedding_bias, mean, rstd = ctx.saved_tensors
        original_shape = ctx.original_shape
        
        # Quantize embedding weights for backward pass
        quantized_embedding_weight = weight_quant(embedding_weight).to(x.device)
        
        # Perform embedding lookup with quantized weights
        # print(f"x.device: {x.device}, quantized_embedding_weight.device: {quantized_embedding_weight.device}")
        embeddings = F.embedding(x, quantized_embedding_weight)
        
        # Reshape grad_output and embeddings to 2D
        grad_output_2d = grad_output.view(-1, grad_output.size(-1))
        embeddings_2d = embeddings.view(-1, embeddings.size(-1))
        
        # Apply backward pass using Triton kernel
        dx, dw, db, _, _ = _layer_norm_bwd(
            grad_output_2d, embeddings_2d, norm_weight, norm_bias, ctx.eps, mean, rstd,
            dresidual=None, has_residual=False, is_rms_norm=False,
            x_dtype=embeddings.dtype, recompute_output=True
        )
        ...
        grad_embedding_weight = torch.zeros_like(embedding_weight).to(x.device)
        
        # print(f"grad_embedding_weight.device: {grad_embedding_weight.device}")
        # print(f"dx.device: {dx.device}")
        # print(f"x.device: {x.device}")
        grad_embedding_weight.index_add_(0, x.view(-1), dx.view(-1, dx.size(-1)))
...
def replace_embeddings_in_hf(model):
    """
    Replaces all instances of nn.Embedding in Hugging Face transformers with BitEmbedding158b.

    Args:
        model (nn.Module): The transformer model to modify.
        ...
        if isinstance(module, nn.Embedding):
            ...
            setattr(
                ...
                BitEmbedding158b(
                    ...
                    bias=True,  # Assuming bias=True as default; adjust as necessary
                ),
            )
        else:
            # Recursively apply to child modules
            replace_embeddings_in_hf(module)

def replace_embeddings_in_pytorch_model(model):
    """
    Replaces all instances of nn.Embedding in the given PyTorch model with BitEmbedding158b.
    ...
    for name, module in model.named_children():
        if isinstance(module, nn.Embedding):
            ...
            # Recursively apply to child modules
            replace_embeddings_in_pytorch_model(module)